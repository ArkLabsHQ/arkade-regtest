...
// Ensure the node has a loaded wallet with spendable coins. The bitcoind image
// auto-creates the default wallet and mines 101 blocks on first init; this is a
// defensive top-up for any state where that didn't leave spendable balance.
export function bootstrapChain() {
  // Make sure the default wallet exists and is loaded (ignore "already" errors).
  bitcoinCli(['createwallet', ''], { capture: true });
  bitcoinCli(['loadwallet', ''], { capture: true });
  if (getBalance() < 1) {

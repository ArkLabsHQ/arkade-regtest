...
// Recreate ONLY arkd-wallet with the given signer set, then restart arkd to
// re-read it. docker()/compose() spawn with process.env, so setting the vars
// here is what compose interpolates into the service.
function applySignerSet(active, deprecated) {
  ...
  // arkd caches the signer set at startup, so it must restart to pick up the
  // rotated wallet. It auto-unlocks via the env unlocker, so no admin unlock.
...
export async function rotateSigner({ cutoff, newKey } = {}) {
  ...
  applySignerSet(active, deprecated);
  await sleep(5000);

...
async function verifyPairs() {
  // Hit the Boltz API directly rather than through the nginx CORS proxy, so a
  // health check doesn't depend on that extra hop. Boltz must (re)connect to the
  // ark/fulmine endpoint before it publishes the ARK pair; on cold CI runners
  // that handshake can take a couple of minutes.
  const port = env('BOLTZ_API_PORT', '9001');
  ...
  const ok = await waitFor(
    ...
    async () => {
      ...
      last = `HTTP ${status}: ${text}`;
      return text.includes('"ARK"');
    },
    { attempts: 90, intervalMs: 2000 },
  );
  if (!ok) {
    warn(`Last /v2/swap/submarine response: ${last.slice(0, 600)}`);

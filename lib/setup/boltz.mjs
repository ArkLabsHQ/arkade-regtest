...
import { sleep, waitFor, waitForOrFail } from '../wait.mjs';
...
async function verifyPairs() {
  ...
  const ok = await waitFor('Boltz ARK/BTC pairs', async () => {
    const res = await fetch(`http://localhost:${port}/v2/swap/submarine`, {
      signal: AbortSignal.timeout(15000),
    });
    return (await res.text()).includes('"ARK"');
  });

...
function postIntentFees(fees) {
  return fetchText(`${ADMIN}/v1/admin/intentFees`, {
    method: 'POST',
    headers: JSON_HEADERS,
...
async function setupArkClient() {
  ...
  if (dockerExec('arkd', ['ark', 'config'], { capture: true }).code !== 0) {
    ...
    const init = dockerExec(
      'arkd',
      ['ark', 'init', '--password', password, '--server-url', INFO, '--explorer', explorer],
      { capture: true },
    );
    if (init.code !== 0) {
  ...
  await postIntentFees(FEE_FREE); // redeem intent must cover the fee; zero it briefly
...
export async function setupArkd() {
  ...
  await setupArkClient(); // init + fund (runs with intent fees zeroed)
  await setupFees(); // apply the configured intent fees last
}
